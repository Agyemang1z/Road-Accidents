###BUILDING ARIMA MODEL WITH R USING DATASET AIRPASSENGERS
## Website for arima write ups
...
## Arima using Seasonalize series
auto.arima(Tseries) # actual data--(2013-2018)
auto.arima(RoadAccident_seasdiff) # seasonalize data
...
SARIMA6<-Arima(Tseries, order = c(1,1,1), 
          seasonal = list(order = c(1,0,0), period = 12), 
          include.mean = TRUE,include.drift = FALSE)
